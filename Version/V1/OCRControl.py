...
class OcrThread(QThread):
    ...
    - Sendet über ein Signal erkannte Positionen und Klick-Parameter an den Mausaktions-Thread.
    - Prüft, ob ein erkannter Name in der Player Blacklist steht. Falls ja und GM Warning aktiviert ist,
      wird der Error-Sound abgespielt.
    """
    log_signal = pyqtSignal(str)  # Sendet Log-Meldungen an die GUI
    status_signal = pyqtSignal(str)  # Aktualisiert den OCR-Status in der GUI
    mouse_action_signal = pyqtSignal(str, tuple, float)  # (name, pos, click_delay)
    ...
    def run(self):
        try:
            ...
            while self.running:
                ...
                if recognized_positions:
                    for name, pos in recognized_positions.items():
                        ...
                        # Zuerst: Überprüfen, ob der erkannte Name in der Player Blacklist enthalten ist.
                        if name in self.shared_settings.get("blacklist_player", []):
                            self.log_signal.emit(f"[SYSTEM] Blacklisted Name erkannt: {name}")
                            if self.shared_settings.get("gm_warning", False):
                                self.play_error_sound()
                            continue

                        # Anschließend: Überprüfe die Whitelists.
                        if name in self.shared_settings.get("whitelist_monster", []):
                            click_delay = self.shared_settings.get("click_delay_monster", 500)
                        elif name in self.shared_settings.get("whitelist_player", []):
                            click_delay = self.shared_settings.get("click_delay_player", 500)
                        else:
                            continue

                        self.log_signal.emit(f"[OCR] ✔ Erkannt: {name} an Position {pos}")
            ...
            self.log_signal.emit("[OCR] 📌 OCR-Thread gestoppt.")
    ...
    def stop(self):
        self.log_signal.emit("[SYSTEM] ⛔ OCR-Thread wird gestoppt.")
        self.running = False
        self.quit()
...
class MouseActionThread(QThread):
    ...
    def perform_mouse_action(self, name, pos, click_delay):
        try:
            x, y = pos
            self.log_signal.emit(f"[KLICK] 🖱️ Mausbewegung: {name} an {pos}")
            pyautogui.moveTo(x, y, duration=0.2)
            self.log_signal.emit(f"[KLICK] 🖱️ Maus bewegt zu {name} an {pos}")
            self.msleep(int(click_delay * 1000))
            pyautogui.click(x, y, button="left")
            self.log_signal.emit(f"[KLICK] 🖱️ Klick auf {name} an {pos} mit Verzögerung {click_delay} Sekunden")
        except Exception:
            self.log_signal.emit(f"[SYSTEM] ❌ Fehler bei Mausaktion:\n{traceback.format_exc()}")
...
class OCRControlWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.filter_dropdown.addItems(["Alle Logs", "Nur OCR", "Nur Klicks", "System"])
    ...
    def select_window(self):
        ...
        if title:
            ...
            if target_windows:
                ...
                self.append_log(f"[SYSTEM] Ausgewähltes Fenster: {title}")
            else:
                self.append_log(f"[SYSTEM] Kein Fenster mit Titel '{title}' gefunden.")
